...
# 4. Find and replace "SulfEnegP9" with some single-word name that has meaning to
...
RawDataDir <- "G:/Data/Metabolomics/Laura/Sulforaphane project/20130605 Sulf EnegP mzData files"

MainDir <- "C:/Users/Laura/Documents/Sulforaphane project/9 SulfEnegP"
...
xs8 <- xcmsSet(Samples[141:144], method = "centWave",  ppm=PPM, 
               peakwidth=c(4,12), 
               snthresh = 15, mzCenterFun="apex", prefilter=c(10, 1000),
               integrate = 1, fitgauss= TRUE)
...
# Peak alignment
SulfEnegP9.grouped <- group(c(xs1, xs2, xs3, xs4, xs5, xs6, xs7, xs8), 
...
rm(xs1, xs2, xs3, xs4, xs5, xs6, xs7, xs8)
...
SulfEnegP9.RTcor <- retcor(SulfEnegP9.grouped, method = "peakgroups", 
...
setwd(MainDir)
png("SulfEnegP9 RTcor plot.png", width = 6, height = 8, units = "in", res = 300)
plotrt(SulfEnegP9.RTcor, leg = F, densplit = T)
dev.off()
setwd(RawDataDir)
...
SulfEnegP9.grouped2 <- group(SulfEnegP9.RTcor, method = "density", 
                             minsamp = 1, minfrac = 0, mzwid = 0.007, 
                             bw = 2, max=100)

# Making a table of the data before the recursive peak filling step
SulfEnegP9.unfilled <- peakTable(SulfEnegP9.grouped2)

setwd(MainDir)
write.csv(SulfEnegP9.unfilled, 
          "SulfEnegP9 peak table - unfilled.csv")
setwd(RawDataDir)
...
SulfEnegP9.filledpeaks <- fillPeaks(SulfEnegP9.grouped2)
...
SulfEnegP9.allpeaks <- peakTable(SulfEnegP9.filledpeaks, 
                                 filebase="SulfEnegP9 peak table")

# Making a column with the mass feature name
SulfEnegP9.allpeaks$MassFeature <- paste("I", round((
      SulfEnegP9.allpeaks$mz),digits=4), "R", round(
            SulfEnegP9.allpeaks$rt/60, digits=2), sep="")

# Making a column with the RT in minutes. Note that this is different
# from the column "rt", which is the RT in seconds. 
SulfEnegP9.allpeaks$RT <- SulfEnegP9.allpeaks$rt/60
...
for (m in 1:length(RefIons)){
      ...
      RefMFs[[m]] <- which(SulfEnegP9.allpeaks$mz < 
                                 (RefIons[m] + PPM/1e6*RefIons[m]) &
                                 SulfEnegP9.allpeaks$mz > 
                                 RefIons[m] - PPM/1e6*RefIons[m])
      
}

# Removing reference ions from the data
SulfEnegP9.noref <- SulfEnegP9.allpeaks[-unlist(RefMFs), ]

setwd(MainDir)
write.csv(SulfEnegP9.noref, "SulfEnegP9 peak table without ref ions.csv")

# Retaining only mass features that elute after 2 min since our RT isn't 
# very reproducible before 2 minutes.
SulfEnegP9.after2 <- subset(SulfEnegP9.noref, SulfEnegP9.noref$rt 
                            > 120)
write.csv(SulfEnegP9.after2, 
          "SulfEnegP9 peak table - RT above 2 min.csv")

# Counting how many times a mass feature was detected BEFORE recursive peak
# filling step.
Count.df <- SulfEnegP9.unfilled[as.numeric(row.names(
      SulfEnegP9.after2)), SampCol]

SulfEnegP9.after2$Count <- rep(NA, nrow(SulfEnegP9.after2))

for (m in 1:nrow(Count.df)){
      SulfEnegP9.after2$Count[m] <- length(which(!is.na(as.numeric(
            Count.df[m, ]))))
}

# Only retaining mass features that were detected in the initial peak-picking 
# step in at least 25% of all samples.
SulfEnegP9.filter <- SulfEnegP9.after2[SulfEnegP9.after2$Count >= 
                                             0.25*length(SampCol), ]
...
MFs <- as.numeric(sample(row.names(SulfEnegP9.filter), 30))
RandSamp <- as.numeric(sample(length(Samples), 10))
write.csv(MFs, paste(Sys.Date(), 
                     "SulfEnegP9 randomly selected mass features.csv"))
write.csv(RandSamp, paste(Sys.Date(), 
                          "SulfEnegP9 randomly selected samples.csv"))
...
for (m in MFs){
      EIC.uncorrected[[m]] <- getEIC(SulfEnegP9.filledpeaks, rt = "raw", 
                                     groupidx = m, sampleidx = RandSamp)
      EIC.corrected[[m]] <- getEIC(SulfEnegP9.filledpeaks, rt = "corrected", 
                                   groupidx = m, sampleidx = RandSamp)  
}
...
setwd(MainDir)
pdf("SulfEnegP9 peak picking and alignment quality check.pdf", 8.5, 11)
...
for(i in 1:30){
      m <- MFs[i]
      plot(EIC.uncorrected[[m]], SulfEnegP9.filledpeaks, groupidx = 1, 
           rtrange = 60, col = MyColors, main = MFs[m])
      mtext(paste(i, SulfEnegP9.allpeaks$MassFeature[m]), side = 3, line = -1, 
            adj = 0, padj = 0, cex = 0.8)
      plot(EIC.corrected[[m]], SulfEnegP9.filledpeaks, groupidx = 1, 
           rtrange = 60, col = MyColors)
      
      RT <- SulfEnegP9.allpeaks$rt[m]
      RTRange <- c(RT - 30, RT + 30)
      
      mz <- SulfEnegP9.allpeaks$mz[m]
      ...
      mtext(paste("abund =", round(SulfEnegP9.allpeaks[m, (length(RandSamp))], 
                                   digits = 0)), 
...
SulfEnegP9.xsa <- xsAnnotate(SulfEnegP9.filledpeaks)

# Find the compound groups based on RT
SulfEnegP9.xsaF <- groupFWHM(SulfEnegP9.xsa, perfwhm = 1.5)

# Check that peaks in the same group correlate well enough to continue to be 
# included. Generate pseudospectra.
SulfEnegP9.xsaC <- groupCorr(SulfEnegP9.xsaF, cor_eic_th = 0.85, 
                             pval=0.0001, calcIso=TRUE, calcCiS=TRUE)

# Find the isotopes within each pseudospectrum.
SulfEnegP9.xsaFI <- findIsotopes(SulfEnegP9.xsaC, maxcharge = 3, maxiso = 8, 
                                 ppm = 10, mzabs = 0.015, intval = "maxo", 
                                 minfrac = 0.25)

# Find the adducts within each pseudospectrum.
SulfEnegP9.xsaFA <- findAdducts(SulfEnegP9.xsaFI, ppm = 10, mzabs = 0.015, 
                                multiplier = 3, polarity = IonizationMode, 
                                rules = NULL, max_peaks = 100)

# Make a table of the annotated peaks.
SulfEnegP9.annot <- getPeaklist(SulfEnegP9.xsaFA)

# Once again, remove reference ions from the peak table.
MF.refs <- list()

for (i in 1:length(RefIons)){
      MF.refs[[i]] <- which(SulfEnegP9.annot$mz < 
                                  (RefIons[i] + PPM/1e6*RefIons[i]) &
                                  SulfEnegP9.annot$mz > 
                                  RefIons[i] - PPM/1e6*RefIons[i])
}

SulfEnegP9.annot <- SulfEnegP9.annot[-unlist(MF.refs), ]

# Save the original annotated peak list.
setwd(MainDir)
write.csv(SulfEnegP9.annot, file = "SulfEnegP9 annotated peaklist.csv")
setwd(RawDataDir)

# Make the column "isotopes" be character data instead of the default, factor.
# Replace empty cells with "NA".
SulfEnegP9.annot$isotopes <- as.character(SulfEnegP9.annot$isotopes)
SulfEnegP9.annot$isotopes[SulfEnegP9.annot$isotopes == ""] <- NA

# Processing CAMERA data to remove redundant, isotopic mass features.
# Making a column with the mass feature name.
SulfEnegP9.annot$MassFeature <- paste0("I", round((SulfEnegP9.annot$mz),
                                                  digits = 4), "R", 
                                       round(SulfEnegP9.annot$rt/60, 
                                             digits = 2))

# Making a column with the RT in minutes. Note that this is different
# from the column "rt", which is the RT in seconds. 
SulfEnegP9.annot$RT <- round(SulfEnegP9.annot$rt/60, digits = 2)

# Take a subset of the data that is only the mass features that appear
# to have isotopes. 
Iso <- SulfEnegP9.annot[, c("MassFeature", "mz", "RT", "isotopes", 
                            "pcgroup")]
...
Iso <- merge(Iso, SulfEnegP9.annot, by=c("MassFeature", "mz", "RT", 
                                         "isotopes", "pcgroup"), 
             type = "left")
...
IsoCollapse <- join(SulfEnegP9.annot[, c("MassFeature", "mz", "RT", 
                                         "pcgroup",
                                         "isotopes", "adduct")], 
                    IsoCollapse, 
...
# as "NA".
SulfEnegP9.collapse <- SulfEnegP9.annot[is.na(SulfEnegP9.annot$isotopes), ]
SulfEnegP9.collapse$IsoGroup <- rep(NA, nrow(SulfEnegP9.collapse))

# Add rows to the SulfEnegP9.collapse data.frame that are the collapsed 
# isotopic mass features.
SulfEnegP9.collapse <- rbind(SulfEnegP9.collapse[, c(
...
SulfEnegP9.filter2 <- join(SulfEnegP9.after2[, c("MassFeature", 
                                                 "Count")],
                           SulfEnegP9.collapse, by = "MassFeature", 
                           type = "right")
SulfEnegP9.filter2 <- SulfEnegP9.filter2[
      complete.cases(SulfEnegP9.filter2$Count), ]

# Retain only mass features with RT >= 2 minutes.
SulfEnegP9.filter2 <- SulfEnegP9.filter2[
      SulfEnegP9.filter2$Count >= 0.25*length(SampCol) & 
            SulfEnegP9.filter2$RT >= 2, ]

setwd(MainDir)
write.csv(SulfEnegP9.filter2, "SulfEnegP9 - isotopes collapsed.csv")
setwd(RawDataDir)
...
PS <- as.numeric(unique(SulfEnegP9.filter2$pcgroup[
      complete.cases(SulfEnegP9.filter2$isotopes)]))

# Randomly select 32 pseudospectra to scrutinize.
PS.rand <- sample(PS, 32)

setwd(MainDir)
pdf("SulfEnegP9 CAMERA quality check.pdf", height=11, width=8.5)
par(mfrow=c(4,2))

# For each pseudospectrum, plot the EICs and the mass spectrum.
for (p in PS.rand){
      
      plotEICs(SulfEnegP9.xsaFA, pspec=p, maxlabel=5)      
      plotPsSpectrum(SulfEnegP9.xsaFA, pspec=p, maxlabel=5) 
...
setwd(MainDir)
write.csv(rbind(Times, TotalTime), "SulfEnegP9 processing times.csv", row.names=F)
...
ggplot(Times, aes(x=Step, y=Duration, fill=Step)) + geom_bar(stat="identity") +
      ylab("Duration (min)") + ggtitle("Processing times for SulfEnegP9") +
      guides(fill=FALSE)
ggsave("SulfEnegP9 processing times bar plot.png", height=6, width=6.5, dpi=300)
...
ggplot(Times[-1,], aes(x=Step, y=Duration, fill=Step)) + 
      ...
      ylab("Duration (min)") + ggtitle("Processing times for SulfEnegP9") +
      guides(fill=FALSE)
ggsave("SulfEnegP9 processing times without peak picking step - bar plot.png", 
...
MFCount <- data.frame("Step" = c("Number of MFs initially", 
                      ...
                      "Count" = c(nrow(SulfEnegP9.unfilled),
                                  nrow(SulfEnegP9.noref),
                                  nrow(SulfEnegP9.after2),
                                  nrow(SulfEnegP9.filter),
                                  nrow(SulfEnegP9.filter2)))


MFCount$Step <- factor(MFCount$Step, levels = MFCount$Step)

write.csv(MFCount, "SulfEnegP9 counts of mass features at each step.csv", 
          row.names=F)
...
ggsave("SulfEnegP9 bar chart of numbers of mass features at each step.png")
