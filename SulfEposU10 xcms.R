...
# Peak alignment
SulfEposU10.grouped <- group(c(xs1, xs2, xs3, xs4, xs5, xs6, xs7, xs8), 
...
SulfEposU10.RTcor <- retcor(SulfEposU10.grouped, method = "peakgroups", 
...
setwd(MainDir)
png("SulfEposU10 RTcor plot.png", width = 6, height = 8, units = "in", res = 300)
plotrt(SulfEposU10.RTcor, leg = F, densplit = T)
dev.off()
setwd(RawDataDir)
...
SulfEposU10.grouped2 <- group(SulfEposU10.RTcor, method = "density", 
                              minsamp = 1, minfrac = 0, mzwid = 0.007, 
                              bw = 2, max=100)

# Making a table of the data before the recursive peak filling step
SulfEposU10.unfilled <- peakTable(SulfEposU10.grouped2)

setwd(MainDir)
write.csv(SulfEposU10.unfilled, 
          "SulfEposU10 peak table - unfilled.csv")
setwd(RawDataDir)
...
SulfEposU10.filledpeaks <- fillPeaks(SulfEposU10.grouped2)
...
SulfEposU10.allpeaks <- peakTable(SulfEposU10.filledpeaks, 
                                  filebase="SulfEposU10 peak table")
...
setwd(MainDir)
write.csv(SulfEposU10.noref, "SulfEposU10 peak table without ref ions.csv")
...
SulfEposU10.after2 <- subset(SulfEposU10.noref, SulfEposU10.noref$rt 
                             > 120)
write.csv(SulfEposU10.after2, 
          "SulfEposU10 peak table - RT above 2 min.csv")

# Counting how many times a mass feature was detected BEFORE recursive peak
# filling step.
Count.df <- SulfEposU10.unfilled[as.numeric(row.names(
      SulfEposU10.after2)), SampCol]

SulfEposU10.after2$Count <- rep(NA, nrow(SulfEposU10.after2))

for (m in 1:nrow(Count.df)){
      SulfEposU10.after2$Count[m] <- length(which(!is.na(as.numeric(
            Count.df[m, ]))))
}

# Only retaining mass features that were detected in the initial peak-picking 
# step in at least 25% of all samples.
SulfEposU10.filter <- SulfEposU10.after2[SulfEposU10.after2$Count >= 
                                               0.25*length(SampCol), ]
...
save.image("SulfEposU10 workspace.RData")
...
MFs <- as.numeric(sample(row.names(SulfEposU10.filter), 30))
...
for (m in MFs){
      EIC.uncorrected[[m]] <- getEIC(SulfEposU10.filledpeaks, rt = "raw", 
                                     groupidx = m, sampleidx = RandSamp)
      EIC.corrected[[m]] <- getEIC(SulfEposU10.filledpeaks, rt = "corrected", 
                                   groupidx = m, sampleidx = RandSamp)  
}
...
setwd(MainDir)
pdf("SulfEposU10 peak picking and alignment quality check.pdf", 8.5, 11)
...
for(i in 1:30){
      m <- MFs[i]
      plot(EIC.uncorrected[[m]], SulfEposU10.filledpeaks, groupidx = 1, 
           rtrange = 60, col = MyColors, main = MFs[m])
      mtext(paste(i, SulfEposU10.allpeaks$MassFeature[m]), side = 3, line = -1, 
            adj = 0, padj = 0, cex = 0.8)
      plot(EIC.corrected[[m]], SulfEposU10.filledpeaks, groupidx = 1, 
           rtrange = 60, col = MyColors)
      
      RT <- SulfEposU10.allpeaks$rt[m]
      RTRange <- c(RT - 30, RT + 30)
      
      mz <- SulfEposU10.allpeaks$mz[m]
      ...
      mtext(paste("abund =", round(SulfEposU10.allpeaks[m, (length(RandSamp))], 
                                   digits = 0)), 
...
SulfEposU10.xsa <- xsAnnotate(SulfEposU10.filledpeaks)

# Find the compound groups based on RT
SulfEposU10.xsaF <- groupFWHM(SulfEposU10.xsa, perfwhm = 1.5)

# Check that peaks in the same group correlate well enough to continue to be 
# included. Generate pseudospectra.
SulfEposU10.xsaC <- groupCorr(SulfEposU10.xsaF, cor_eic_th = 0.85, 
                              pval=0.0001, calcIso=TRUE, calcCiS=TRUE)

# Find the isotopes within each pseudospectrum.
SulfEposU10.xsaFI <- findIsotopes(SulfEposU10.xsaC, maxcharge = 3, maxiso = 8, 
                                  ppm = 10, mzabs = 0.015, intval = "maxo", 
                                  minfrac = 0.25)

# Find the adducts within each pseudospectrum.
SulfEposU10.xsaFA <- findAdducts(SulfEposU10.xsaFI, ppm = 10, mzabs = 0.015, 
                                 multiplier = 3, polarity = IonizationMode, 
                                 rules = NULL, max_peaks = 100)

# Make a table of the annotated peaks.
SulfEposU10.annot <- getPeaklist(SulfEposU10.xsaFA)

# Once again, remove reference ions from the peak table.
MF.refs <- list()

for (i in 1:length(RefIons)){
      MF.refs[[i]] <- which(SulfEposU10.annot$mz < 
                                  (RefIons[i] + PPM/1e6*RefIons[i]) &
                                  SulfEposU10.annot$mz > 
                                  RefIons[i] - PPM/1e6*RefIons[i])
}

SulfEposU10.annot <- SulfEposU10.annot[-unlist(MF.refs), ]

# Save the original annotated peak list.
setwd(MainDir)
write.csv(SulfEposU10.annot, file = "SulfEposU10 annotated peaklist.csv")
setwd(RawDataDir)

# Make the column "isotopes" be character data instead of the default, factor.
# Replace empty cells with "NA".
SulfEposU10.annot$isotopes <- as.character(SulfEposU10.annot$isotopes)
SulfEposU10.annot$isotopes[SulfEposU10.annot$isotopes == ""] <- NA

# Processing CAMERA data to remove redundant, isotopic mass features.
# Making a column with the mass feature name.
SulfEposU10.annot$MassFeature <- paste0("I", round((SulfEposU10.annot$mz),
                                                   digits = 4), "R", 
                                        round(SulfEposU10.annot$rt/60, 
                                              digits = 2))

# Making a column with the RT in minutes. Note that this is different
# from the column "rt", which is the RT in seconds. 
SulfEposU10.annot$RT <- round(SulfEposU10.annot$rt/60, digits = 2)

# Take a subset of the data that is only the mass features that appear
# to have isotopes. 
Iso <- SulfEposU10.annot[, c("MassFeature", "mz", "RT", "isotopes", 
                             "pcgroup")]
...
Iso <- merge(Iso, SulfEposU10.annot, by=c("MassFeature", "mz", "RT", 
                                          "isotopes", "pcgroup"), 
             type = "left")
...
IsoCollapse <- join(SulfEposU10.annot[, c("MassFeature", "mz", "RT", 
                                          "pcgroup",
                                          "isotopes", "adduct")], 
                    IsoCollapse, 
...
# as "NA".
SulfEposU10.collapse <- SulfEposU10.annot[is.na(SulfEposU10.annot$isotopes), ]
SulfEposU10.collapse$IsoGroup <- rep(NA, nrow(SulfEposU10.collapse))

# Add rows to the SulfEposU10.collapse data.frame that are the collapsed 
# isotopic mass features.
SulfEposU10.collapse <- rbind(SulfEposU10.collapse[, c(
...
SulfEposU10.filter2 <- join(SulfEposU10.after2[, c("MassFeature", 
                                                   "Count")],
                            SulfEposU10.collapse, by = "MassFeature", 
                            type = "right")
SulfEposU10.filter2 <- SulfEposU10.filter2[
      complete.cases(SulfEposU10.filter2$Count), ]

# Retain only mass features with RT >= 2 minutes.
SulfEposU10.filter2 <- SulfEposU10.filter2[
      SulfEposU10.filter2$Count >= 0.25*length(SampCol) & 
            SulfEposU10.filter2$RT >= 2, ]

setwd(MainDir)
write.csv(SulfEposU10.filter2, "SulfEposU10 - isotopes collapsed.csv")
setwd(RawDataDir)
...
PS <- as.numeric(unique(SulfEposU10.filter2$pcgroup[
      complete.cases(SulfEposU10.filter2$isotopes)]))

# Randomly select 32 pseudospectra to scrutinize.
PS.rand <- sample(PS, 32)

setwd(MainDir)
pdf("SulfEposU10 CAMERA quality check.pdf", height=11, width=8.5)
par(mfrow=c(4,2))

# For each pseudospectrum, plot the EICs and the mass spectrum.
for (p in PS.rand){
      
      plotEICs(SulfEposU10.xsaFA, pspec=p, maxlabel=5)      
      plotPsSpectrum(SulfEposU10.xsaFA, pspec=p, maxlabel=5) 
...
setwd(RawDataDir)
save.image("SulfEposU10 workspace.RData") # This saves EVERYTHING that is currently
